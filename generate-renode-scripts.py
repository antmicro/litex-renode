...
""".format(
        generate_sysbus_registration(peripheral['address'],
...
def generate_repl():
    ...
    name_to_handler = {
        ...
        'ethmac': {
            ...
            'phy': lambda: dict(configuration.peripherals['ethphy'].items() + {'size': 0x800}.items())
        },
        'cas': {
...
def filter_memory_regions(raw_regions, alignment=None):
    """ Filters memory regions skipping those
        from `non_generated_mem_regions` list
        and verifying if they have proper size and
        do not overlap.

        Args:
            raw_regions (list): list of memory regions
                                parsed from configuration
                                file
        alignment (int or None): memory size boundary

        Returns:
            list: reduced, sorted list of memory regions
                  to be generated in a repl file
    """
